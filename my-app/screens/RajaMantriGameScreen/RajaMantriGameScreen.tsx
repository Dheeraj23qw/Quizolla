...
const RajaMantriGameScreen: React.FC<RajaMantriGameScreenProps> = () => {
  ...
  const handlePlay = () => {
    ...
    if (policeIndex !== -1) {
      ...
      setMessage(`Player ${policePlayerName}, you are chosen for the Police role. Find out who is the Thief!`);

      flipCard(policeIndex, 1, 1800);

      setTimeout(() => {
        flipCard(policeIndex, 0, 1800);
        setIsPlayButtonDisabled(false);
        setMessage('Choose a card to reveal the roles!');
      }, 12000);
    }
  };
  ...
  const resetForNextRound = () => {
    if (round >= 10) {
      ...
      setClickedCards(initialClickedCards);
      setIsPlayButtonDisabled(false);
      setMessage('New round! Choose a player to press the button.');
        ...
        <View style={styles.table}>
          {playerNames.map((name, index) => (
            <View style={styles.tableRow} key={index}>
              ...
              <View style={styles.tableCell}>
                <Text style={styles.cellText}>{points[index]}</Text>
              </View>
            </View>
          ))}
        </View>
